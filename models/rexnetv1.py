...
class ReXNetV1(nn.Module):
    def __init__(self, input_ch=3, final_ch=512, width_mult=1.0, depth_mult=1.0, classes=1000,use_se=True,se_ratio=12,dropout_ratio=0.2,bn_momentum=0.9):
        ...
        ConvBNSiLU(features, input_ch, int(round(stem_channel * width_mult)), kernel=3, stride=2, pad=1)
...
if __name__ == '__main__':
    model = ReXNetV1(3,width_mult=1.0).cuda()
    out = model(torch.randn(128, 3, 224, 224).cuda())
    loss = out.sum()
    loss.backward()
